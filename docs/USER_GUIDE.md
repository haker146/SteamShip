# SteamShip User Guide

## Modern UI

SteamShip 5.5.0 introduced a new browser-based interface built with QWebEngine. It launches by default alongside the classic interface. Navigate using the sidebar on the left.
...
**Library** — shows all games installed across your Steam libraries. Use the SteamShip Only filter to show games that have a saved Lua or `config/stplug-in` Lua entry.
...
**Cloud Saves** — enter your Steam path and Steam32 ID, then scan. SteamShip scans Steam userdata and bundled Ludusavi save paths, groups all save folders for the same App ID into one backup, and restores each source with per-location results. A safety backup is created before any overwrite.

Providers:

- **Local folder** — pick any folder on your machine or network drive.
- **Google Drive** — select Google Drive in the provider grid and click Connect. Sign in once in the browser that opens. Backups go to a `SteamShip Backups/` folder in your Drive.
- **rclone** — supports 70+ cloud backends including Dropbox, OneDrive, MEGA, pCloud, Backblaze B2, Amazon S3, SFTP, and more. Setup flow:
  1. Select rclone in the provider grid. SteamShip auto-fills the bundled `rclone.exe` path.
...
**5. ACF writing**
Creates or overwrites the .acf file for the game. ACF files tell Steam the state of a game installation. If SteamShip asks "Are you updating a game you already have installed or is this a new installation?", choose "I'm updating a game" to skip rewriting it, or "New installation" to overwrite it.
...
**1. Version selection**
Browse the Store tab, find the game, and pick the version you want. SteamShip fetches the available depot/manifest combinations from the source. If that list is empty, use the version picker’s Manual Depot / Manifest IDs box, or click Import HTML and choose saved SteamDB depot pages so SteamShip lists their newest depot/manifest pairs for selection.

**2. Lua download**
SteamShip automatically downloads the Lua file for the selected game (from Hubcap or OurEveryday, depending on your selection).
...
**7. ACF writing**
SteamShip writes the ACF file with:
...
### Scan game library
Scans all your Steam libraries and lists your installed games. The modern Library tab can filter to SteamShip-managed games by comparing installed App IDs with saved Lua files and Steam `config/stplug-in` Lua files.
...
- **PRE-INSTALLED** — no depot needed, just add the DLC ID to the stplug-in Lua file (SteamShip can do this for you)
...
Searches online-fix.me for your game and opens the best result in your browser. SteamShip does not store online-fix.me credentials or download files from the site. See [Multiplayer Fix](MULTIPLAYER_FIX.md) for more detail.


### Fixes & Bypasses
Searches a curated fix list for a fix or bypass for your game. No account needed — SteamShip fetches the list, lets you search with fuzzy matching, downloads the fix, and extracts it straight into the game folder. This is a second source of fixes that often covers games not found on online-fix.me. See [Fixes & Bypasses](CRACK_FIX.md) for more detail.
...
### Check for updates
Checks GitHub for the latest SteamShip release and compares it to your version. If no update exists, the button stops loading and tells you the app is already up to date.

### Install/Uninstall Context Menu
Adds or removes a right-click option on .lua and .zip files in Windows Explorer that opens SteamShip directly into the "Process a .lua file" step with that file already loaded.
...
### Settings
Edit, export, or import SteamShip settings. Settings are usually set automatically as you use the tool, but you can change Steam path, API keys, custom appearance, live log limit, and feature toggles here. Use the Settings Backup section at the top to export a JSON file or import one back. The modern export skips saved secrets such as API keys.
...
| File | Purpose |
|---|---|
| `settings.bin` | All SteamShip settings (encrypted where needed) |
...
- **Use full game names** when searching online-fix.me (e.g. "Counter-Strike: Global Offensive" not "CS:GO"). SteamShip opens the result in your browser and leaves downloads to the website.
...
- **Antivirus** may flag files downloaded by SteamShip (false positives are common with game-related tools). Exclude the SteamShip folder and `sff\dlc_unlockers\resources` from Windows Security if needed.
