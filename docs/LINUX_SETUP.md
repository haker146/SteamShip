...
SteamShip runs natively on Linux but the architecture differs from Windows. This page covers everything specific to the Linux build.

## TL;DR

- **LumaCore is Windows-only.** It is a Steam-client DLL hijack via `dwmapi.dll`. There is no Linux equivalent shipping today.
- On Linux SteamShip uses **SLSsteam** for license/family-share injection and **SLScheevo** for achievements. Both run as a `LD_AUDIT`-style injection into the Steam process.
- Manifests, depot keys, ACF writing, depot decryption, and the rest of the SteamShip pipeline work the same as Windows.
...
| Download via DepotDownloaderMod | Works | Needs `.NET 9` on PATH. SteamShip installs it via the Linux setup tool if missing. |
...
Two options:
- **AppImage** (`SteamShip-x.x.x-linux.AppImage`): chmod +x, double-click. Easiest path.
- **ZIP** (`SteamShip-x.x.x-linux.zip`): extract, run `./SteamShip_GUI`.

Both bundle Python and the GUI runtime. No system Python needed.

### 2. Install SLSsteam + SLScheevo

Open SteamShip, go to **Quick Tools**, click **Set up Linux tools (SLSsteam + .NET 9)**.
...
If you already have `.NET 9` somewhere unusual (e.g. `~/.dotnet/dotnet`), SteamShip picks it up via the user `dotnet` config or the standard `~/.dotnet` path.

### 3. Restart Steam from SteamShip

This is the step Linux users miss most often. SLSsteam injects via `LD_AUDIT` at startup, so a Steam process started outside SteamShip is **not injected** and ownership/family-bypass will not work.
...
Steam was started without SLSsteam injected. Use the SteamShip **Restart Steam** button to relaunch with injection.
...
- Use the SteamShip setup tool's `.NET 9` step (it fetches the Microsoft installer for your distro).
...
3. Restart Steam from SteamShip (not from a desktop icon).

### Steam Deck specific

- Run from Desktop Mode, not Game Mode. The injection cannot attach in Game Mode.
- Steam Deck's read-only root means you cannot install `.NET 9` system-wide. Use the user-local `~/.dotnet/dotnet` install path; the SteamShip setup tool defaults to this.
...
- The contents of the Logs panel from inside SteamShip (last 50 lines is plenty)
