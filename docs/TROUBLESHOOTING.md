...
4. Click OK, then **fully close SteamShip from Task Manager** and reopen it.
...
### Step 3 — Add SteamShip to Windows Defender exclusions
...
3. Add the entire SteamShip folder (e.g. `C:\SteamShip\`) as a **Folder** exclusion.

Restart SteamShip after adding the exclusion.

---

### Step 4 — Run SteamShip as Administrator

Right-click `SteamShip_GUI.exe` → **Run as administrator**. Some sandbox/permission configurations prevent Chromium sub-processes from launching without elevated rights.

---

### Step 5 — Diagnose with Task Manager

Open **Task Manager** while SteamShip is running and look for a process called `QtWebEngineProcess.exe`:
...
Press `Win + R` → type `eventvwr` → **Windows Logs** → **Application**. Look for crash or error entries mentioning `QtWebEngineProcess` or `SteamShip_GUI`. The error description will identify whether it is a missing DLL, access-denied, or GPU driver issue.
...
SteamShip handles this automatically, but if you still see the error:

1. **Workshop ACF fix** — The most common cause is orphaned workshop items in `appworkshop_{id}.acf` triggering a failed Workshop update. SteamShip patches this file to clear `NeedsDownload` when no workshop content is installed.
...
3. **ACF error state** — SteamShip clears stale `UpdateResult` and validation flags in the game ACF so Steam doesn't get stuck in a retry loop.

---

## Steam path not found

SteamShip needs to know where Steam is installed. If it cannot find it automatically, it will ask you to choose the folder. Pick the folder that contains `steam.exe` (usually `C:\Program Files (x86)\Steam`).
...
SteamAutoCrack is bundled inside the `_internal/` folder (shipped alongside `SteamShip_GUI.exe`) and is found automatically. You do **not** need to download or extract it manually.

If you still see this error:
- Make sure you are using the latest version of SteamShip.
- Install SteamShip in a short path like `C:\SFF\` — very long install paths can cause Windows path-length errors (260-character limit).
...
## Antivirus flags SteamShip files

Starting with version 5.3.0, SteamShip uses the one-dir distribution format. All files are pre-extracted into the `_internal/` folder next to `SteamShip_GUI.exe` at install time. Nothing is extracted to `%TEMP%` at runtime, which greatly reduces false positives.

If your antivirus still flags files, add your SteamShip folder (e.g. `C:\SteamShip\`) to your AV exclusions. The `_internal/` folder contains Python runtime files and bundled tools — none of them are malware.

---

## online-fix.me page does not open

SteamShip opens online-fix.me pages in your default browser. If nothing opens, set a default browser in Windows Settings and try again. You can also open online-fix.me yourself and search the full game name.

---

## online-fix.me asks for login or blocks access

SteamShip no longer stores online-fix.me credentials or downloads files from that site. Log in, download, and follow instructions on the website manually.

---

## Download timeout or extraction failed

Check your internet connection. Try disabling antivirus temporarily and run SteamShip again. Make sure you have 7-Zip or WinRAR installed if SteamShip needs to extract archives. For online-fix.me pages, download and extract from your browser because SteamShip only opens the page.

---

## Permission denied or access denied

Steam or the game folder may be in a protected location. Try running SteamShip as administrator (right-click → "Run as administrator"). Do not run SteamShip from a folder that requires admin rights to write to.

---

## Settings export or import error

If exporting settings fails, make sure the folder you export to is writable. The modern Export Settings button skips saved secrets such as API keys. If importing fails, make sure the file is a SteamShip settings JSON and was not edited into invalid JSON.
...
You can delete `api_cache.json` — SteamShip will create a new one when needed. Backup retention is set in Settings; you can lower how many backups are kept.
...
Read the error message first — it often explains what went wrong. Check `debug.log` in the SteamShip folder for more detail.
