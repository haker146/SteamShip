...
This document lists third-party tools, binaries, assets, APIs, services, and projects used by, bundled with, integrated with, downloaded by, or referenced by SteamShip.

SteamShip’s GPL license applies to SteamShip’s own source code only. It does not relicense third-party tools, binaries, unlockers, emulators, assets, APIs, services, or external projects.
...
Notes: DDMod remains owned by its original author/maintainer and is not licensed as SteamShip code.
...
Notes: SteamAutoCrack remains owned by its original author/maintainer and is not licensed as SteamShip code.
...
Notes: fzf remains owned by its original authors and is not licensed as SteamShip code.
...
Notes: gbe_fork remains owned by its original authors and is not licensed as SteamShip code.
...
Notes: gbe_fork Linux components remain owned by their original authors and are not licensed as SteamShip code.
...
Notes: These tools remain owned by their original authors and are not licensed as SteamShip code.
...
Notes: Steamless remains owned by its original author and is not licensed as SteamShip code.
...
Notes: rclone remains owned by its original authors and is not licensed as SteamShip code.
...
Bundled: Yes, where included in SteamShip builds
Modified: No
Used for: Downloading files where aria2 support is used.
Notes: aria2 remains owned by its original authors and is not licensed as SteamShip code.
...
Notes: coldloader remains owned by its original authors and is not licensed as SteamShip code. If a more specific upstream author/source should be credited here, open an issue with the exact source and it will be corrected.
...
Notes: HyperVisor-related third-party files remain owned by their original authors and are not licensed as SteamShip code. If a more specific upstream author/source should be credited here, open an issue with the exact source and it will be corrected.
...
Notes: Linux third-party tools remain owned by their original authors and are not licensed as SteamShip code. If a more specific upstream author/source should be credited here, open an issue with the exact source and it will be corrected.
...
Used for: Linux SLSsteam auto-setup. Downloads and installs the latest SLSsteam release, patches Steam's steam.sh with LD_AUDIT, and configures SLSsteam. Used as the primary Linux setup method with SteamShip's own installer as fallback.
...
Notes: CreamAPI remains owned by deadmau5 and is not licensed as SteamShip code. SteamShip does not claim authorship or ownership of CreamAPI.
...
Notes: SmokeAPI remains owned by Acidicoala and is not licensed as SteamShip code. SteamShip does not claim authorship or ownership of SmokeAPI.
...
Notes: ScreamAPI remains owned by Acidicoala and is not licensed as SteamShip code. SteamShip does not claim authorship or ownership of ScreamAPI.
...
Notes: Uplay R1 Unlocker remains owned by Acidicoala and is not licensed as SteamShip code.
...
Notes: Uplay R2 Unlocker remains owned by Acidicoala and is not licensed as SteamShip code.
...
Notes: SteamShip does not ship CreamInstaller. SteamShip has its own implementation for managing compatible unlocker setups. CreamInstaller remains owned by its original authors/contributors.
...
Notes: SteamShip is not affiliated with online-fix.me. SteamShip does not store online-fix.me credentials or download files from that site.
...
Notes: Hubcap Manifest remains owned/controlled by its maintainers. SteamShip is not affiliated with Hubcap Manifest.
...
Notes: CrakFiles and any referenced fixes remain owned/controlled by their original maintainers/authors. SteamShip only provides tooling around the list and selected user actions.
...
Notes: SteamDB remains owned/controlled by its maintainers. SteamShip is not affiliated with SteamDB.
...
Notes: Google Drive remains owned/controlled by Google. SteamShip is not affiliated with Google.
...
Notes: Each provider remains owned/controlled by its own operator. SteamShip is not affiliated with these providers.
...
Bundled: Yes, where included in SteamShip assets
Modified: No
Used for: MIDI/music asset in SteamShip.
Notes: SteamShip is not affiliated with or endorsed by Team Shanghai Alice, Twilight Frontier, U2 Akiyama, or RedPaper. All trademarks and original works belong to their respective owners.
...
Bundled: Yes
Modified: No
Used for: MIDI/music playback in SteamShip.
...
Notes: miniaudio remains owned by its original author and is not licensed as SteamShip code.
