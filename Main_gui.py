...
try:
    ...
    _msg = (
        "SteamShip failed to start because PyQt6 is missing from the install.\n\n"
        ...
        "https://github.com/haker146/SteamShip/releases/latest\n\n"
    ...
    if sys.platform == "win32":
        try:
            import ctypes
            ctypes.windll.user32.MessageBoxW(0, _msg, "SteamShip — incomplete install", 0x10)
        except Exception:
            pass
    else:
        try:
...
try:
    ...
    QMessageBox.critical(None, "SteamShip startup error", msg[:2000])
    sys.exit(1)
...
def main():
    ...
    app.setApplicationName("SteamShip")
    app.setApplicationDisplayName("SteamShip")
    ...
    _app_icon = QIcon()
    _icon_candidates = list(("steamship.ico", "SFF.ico", "steamship.png", "sff.ico", "SFF.png", "sff.png"))
    # PyInstaller onefile drops resources into _MEIPASS for the running
    # process. relative paths fail when steamship was launched from a
    ...
    if sys.platform == "linux":
        ...
        if _appdir:
            _icon_candidates.insert(0, os.path.join(_appdir, "SteamShip.png"))
            _icon_candidates.insert(0, os.path.join(_appdir, "steamship.png"))
    ...
    if sys.platform == "linux":
        app.setDesktopFileName("steamship")
        ...
        if _appimage:
            try:
                ...
                _icon_dest = _icon_dest_dir / "SteamShip.png"
                _desktop_dir = _home / ".local/share/applications"
                _desktop_file = _desktop_dir / "steamship.desktop"
                _appdir_env = os.environ.get("APPDIR", "")
                _icon_src = Path(_appdir_env) / "SteamShip.png" if _appdir_env else None
    ...
    # A13: explicit "Quit SteamShip" entry on the tray context menu,
    ...
    if _tray_menu is not None:
        _quit_action = _QAction("Quit SteamShip", _tray_menu)
        _quit_action.triggered.connect(_on_tray_quit_steamship)
