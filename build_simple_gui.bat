...
echo ========================================
echo Building SteamShip GUI Executable
echo ========================================
echo.
...
echo ========================================
echo.
echo Folder:     dist\SteamShip_GUI\
echo Executable: dist\SteamShip_GUI\SteamShip_GUI.exe
echo.

if exist "dist\SteamShip_GUI\SteamShip_GUI.exe" (
    python -c "import os; size = sum(os.path.getsize(os.path.join(r,f)) for r,d,files in os.walk('dist/SteamShip_GUI') for f in files); print(f'Total size: {size / (1024*1024):.1f} MB')"
)

echo.
echo You can now run: dist\SteamShip_GUI\SteamShip_GUI.exe
echo Zip the dist\SteamShip_GUI\ folder for distribution.
