/**
 * SteamShip — Main App Router & Sidebar Navigation
...
window.App = (function() {
            ...
            py.get_platform(function(platform) {
                ...
                if (_platform !== 'win32') {
                    Bridge.callWithCallback('get_setting', 'linux_guide_shown', function(val) {
                        if (val === 'True') return;
                        setTimeout(function() {
                            var msg = 'SteamShip has been installed on Linux.\n\n';
        ...
        if (deactivateBtn) {
            deactivateBtn.addEventListener('click', function() {
                var ok = window.confirm(
                    'Deactivate LumaCore?\n\n' +
                    'Steam will be closed first. SteamShip will then remove ' +
    ...
    function _refreshHomeLumacoreNotice(force) {
        ...
        Bridge.callWithCallback('lumacore_check_update', force ? 'force' : '', function(json) {
            ...
            if (!installed) {
                if (titleEl) titleEl.textContent = 'SteamShip runtime required';
                if (bodyEl) bodyEl.textContent = 'Install the SteamShip runtime so Steam can see games added through SteamShip.';
            ...
            if (data.update_available) {
                if (titleEl) titleEl.textContent = 'SteamShip runtime update available';
                if (bodyEl) {
                    bodyEl.textContent = 'Installed ' + installed + ', latest ' + (latest || 'unknown') + '. Update it before adding more games.';
                }
                if (actionEl) actionEl.textContent = 'Update runtime';
                alertEl.classList.remove('hidden');
                return;
            }
    ...
    function _applySteamAccount(account) {
        account = account || {};
        var linked = !!account.linked;
        var name = account.persona || (linked ? 'Steam user' : 'Not signed in');
        var sid = account.steamid64 || '';
        var avatar = account.avatar || 'assets/steamship.png';

        var nameEl = document.getElementById('sidebar-account-name');
        var idEl = document.getElementById('sidebar-account-id');
        var loginBtn = document.getElementById('sidebar-steam-login');
        var avatarEl = document.getElementById('sidebar-account-avatar');
        ...
        if (loginBtn) {
            loginBtn.textContent = linked ? 'Sign out' : 'Sign in with Steam';
            loginBtn.dataset.mode = linked ? 'logout' : 'login';
        }

        var cloudName = document.getElementById('cloud-steam-name');
        var cloudDetail = document.getElementById('cloud-steam-detail');
        var cloudAvatar = document.getElementById('cloud-steam-avatar');
        var cloudLogin = document.getElementById('cloud-steam-login');
        var cloudLogout = document.getElementById('cloud-steam-logout');
        if (cloudName) cloudName.textContent = linked ? name : 'Steam account';
        if (cloudDetail) {
            cloudDetail.textContent = linked
                ? ('Saves go to SteamShip/' + sid + '/')
                : 'Sign in so Drive and rclone backups are keyed to your SteamID64. Without login they go to the unlinked bucket.';
        }
        if (cloudAvatar) cloudAvatar.src = avatar;
        if (cloudLogin) cloudLogin.style.display = linked ? 'none' : '';
        if (cloudLogout) cloudLogout.style.display = linked ? '' : 'none';
    }

    function _refreshSteamAccount() {
        Bridge.callWithCallback('get_steam_account', '', function(json) {
            var account;
            try { account = JSON.parse(json || '{}'); } catch (e) { account = {}; }
            _applySteamAccount(account);
        });
    }

    function _runSteamLogin() {
        Components.showToast('info', 'Complete Steam login in the browser window.');
        Bridge.callWithCallback('steam_login', '', function(json) {
            var result;
            try { result = JSON.parse(json || '{}'); } catch (e) { result = {}; }
            _applySteamAccount(result);
            if (result.ok && result.linked) {
                Components.showToast('success', 'Signed in as ' + (result.persona || result.steamid64));
            } else if (result.error) {
                Components.showToast('error', result.error);
            }
        });
    }

    function _runSteamLogout() {
        Bridge.callWithCallback('steam_logout', '', function(json) {
            var result;
            try { result = JSON.parse(json || '{}'); } catch (e) { result = {}; }
            _applySteamAccount(result);
            Components.showToast('success', 'Steam account unlinked.');
        });
    }

    function _initSteamAccount() {
        _refreshSteamAccount();
        var loginBtn = document.getElementById('sidebar-steam-login');
        if (loginBtn) {
            loginBtn.addEventListener('click', function() {
                if (loginBtn.dataset.mode === 'logout') _runSteamLogout();
                else _runSteamLogin();
            });
        }
        var cloudLogin = document.getElementById('cloud-steam-login');
        if (cloudLogin) cloudLogin.addEventListener('click', _runSteamLogin);
        var cloudLogout = document.getElementById('cloud-steam-logout');
        if (cloudLogout) cloudLogout.addEventListener('click', _runSteamLogout);
    }

    function _setChip(id, text, ok) {
    ...
    function _refreshSetupStrip() {
        Bridge.callWithCallback('get_setting', 'steam_path', function(path) {
            _setChip('chip-steam-path', path ? ('Steam · ' + path) : 'Steam path missing', !!path);
        });
        Bridge.callWithCallback('lumacore_check_update', '', function(json) {
            var data;
            try { data = JSON.parse(json || '{}'); } catch (e) { data = {}; }
            var installed = !!(data.installed);
            _setChip('chip-runtime', installed ? ('Runtime ' + data.installed) : 'Runtime missing', installed);
        });
        var chip = document.getElementById('chip-patterns');
        Bridge.callWithCallback('get_pattern_status', '', function(json) {
            var data;
            try { data = JSON.parse(json || '{}'); } catch (e) { data = {}; }
            var ok = !!(data.toml_ok && data.hooks_ok);
            _setChip('chip-patterns', data.label || 'Patterns unknown', ok);
            if (chip && data.action === 'auto_lc_setup' && !ok) {
                chip.innerHTML = '';
                chip.appendChild(document.createTextNode((data.label || 'Patterns') + ' '));
                var btn = document.createElement('button');
                btn.type = 'button';
                btn.textContent = 'Fix';
                btn.addEventListener('click', function() {
                    var action = document.getElementById('home-lumacore-alert-action');
    ...
    function _initHomeLibraryShelf() {
        Bridge.on('task_finished', function(json) {
            try {
                var data = JSON.parse(json);
                if (data.task === 'library_loaded' && Array.isArray(data.games)) {
                    _renderHomeShelf(data.games);
                }
            } catch (e) {}
        });
    ...
    function _renderHomeShelf(games) {
        var shelf = document.getElementById('home-library-shelf');
        if (!shelf) return;
        shelf.innerHTML = '';
        var list = (games || []).slice(0, 12);
        if (!list.length) {
            shelf.innerHTML = '<p class="page-subtitle">No games yet. Open Store to add one, then Play from Library.</p>';
            return;
        }
        list.forEach(function(game) {
            ...
            img.alt = '';
            img.src = game.image_url || ('https://cdn.cloudflare.steamstatic.com/steam/apps/' + game.app_id + '/header.jpg');
            var title = document.createElement('span');
            title.textContent = game.name || ('App ' + game.app_id);
            card.appendChild(img);
            card.appendChild(title);
            card.addEventListener('click', function() {
                if (game.app_id) Bridge.call('launch_game', String(game.app_id));
            });
