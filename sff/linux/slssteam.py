...
VERSION_FILE = Path.home() / ".local" / "share" / "SteamShip" / "SLSsteam" / "VERSION"
...
def patch_slssteam_config(steam_type: str, print_fn=print) -> bool:
    ...
    try:
        ...
        if patched is not None:
            text = patched
        marker.write_text("patched by SteamShip\n", encoding="utf-8")
    ...
    for i, line in enumerate(lines):
        ...
        if "cloudredirect" in lower and ('="' in line or "='" in line):
            if "cloudredirectdir" in lower or "flatpakcloudredirectdir" in lower:
                ...
                continue
            out.append("# [SteamShip] CloudRedirect line removed\n")
            continue
        ...
        if ("wget" in lower or "curl" in lower) and ("cloud_redirect" in lower or "cloudredirect" in lower):
            out.append("# [SteamShip] CloudRedirect download blocked\n")
            continue
...
def migrate_existing_games(print_fn=print) -> int:
    ...
    Preserves config from tools like ACCELA when users switch to SteamShip."""
    if not _IS_LINUX:
        return 0
