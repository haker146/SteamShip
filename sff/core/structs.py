...
class Settings(Enum):
    ...
    # %APPDATA%\SteamShip\save_backups\.
    ...
    # the work SteamShip registered. Default ON; the off-switch disables
    ...
    # DarkH2o pattern. When ON, SteamShip drops a tiny override .lua into
    ...
    # JSON object {appid: bool}. True means SteamShip has dropped
    ...
    STEAM_OPENID_ID = SettingItem(
        "steam_openid_id",
        "Steam OpenID SteamID64",
        False,
        str,
    )
    STEAM_PERSONA = SettingItem(
        "steam_persona",
        "Steam persona name",
        False,
        str,
    )
    STEAM_AVATAR_URL = SettingItem(
        "steam_avatar_url",
        "Steam avatar URL",
        False,
        str,
    )
    STEAMSHIP_PT_REPO = SettingItem(
        "steamship_pt_repo",
        "SteamShip pattern TOML GitHub repo (owner/name, branch pattern; fallback KoriaPolis/Steam-Auto-PT)",
        False,
        str,
    )
