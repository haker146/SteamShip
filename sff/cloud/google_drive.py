...
_BACKUP_ROOT_NAME = "SteamShip"
_BACKUP_ROOT_FORK = "Steamship"
_BACKUP_ROOT_LEGACY = "SteaMidra Backups"
_META_ROOT_NAME = "_steamship_meta"
_META_ROOT_LEGACY = "_steamidra_meta"
_INTERNAL_BACKUP_FOLDERS = frozenset({_META_ROOT_NAME, _META_ROOT_LEGACY, "_restore_safety"})


def get_backup_root(service):
    from sff.cloud.save_identity import account_bucket
    steamship = get_or_create_folder(service, _BACKUP_ROOT_NAME, "root")
    if not steamship:
        return None
    return get_or_create_folder(service, account_bucket(), steamship)
...
def write_backup_meta(service, backup_root_id, location, folder_name, meta, log_func=None):
    ...
    if not loc_root:
        return False
    tmp = Path(tempfile.mkdtemp(prefix="steamship_drive_meta_"))
    try:
...
def _collect_drive_games(service, root_id, legacy=False):
    games = []
    if not root_id:
        return games
    for item in list_folder(service, root_id):
        ...
        if meta:
            games.append(_drive_backup_entry(item, meta, legacy=legacy))
    return games
...
def list_backup_locations(service):
    from sff.cloud.save_identity import account_bucket
    games = []
    steamship = find_folder(service, _BACKUP_ROOT_NAME, "root")
    if steamship:
        current = find_folder(service, account_bucket(), steamship)
        games.extend(_collect_drive_games(service, current, legacy=False))
        unlinked = find_folder(service, "unlinked", steamship)
        if unlinked and unlinked != current:
            games.extend(_collect_drive_games(service, unlinked, legacy=True))
    legacy_root = find_folder(service, _BACKUP_ROOT_LEGACY, "root")
    if legacy_root:
        games.extend(_collect_drive_games(service, legacy_root, legacy=True))
    if not games:
        return {}
    root_id = steamship or legacy_root
