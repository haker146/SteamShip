...
copies them to %APPDATA%/SteamShip/save_backups/, and tags each backup
...
def _cloud_bucket() -> str:
    try:
        from sff.cloud.save_identity import account_bucket
        return account_bucket()
    except Exception:
        return "unlinked"


def _rclone_keyed_path(remote_dest: str, label: str) -> str:
    from sff.cloud.save_identity import rclone_game_path
    return rclone_game_path(remote_dest, label)
...
def _get_backup_dir():
    ...
    Falls back to <SteamShip install>/save_backups/ otherwise. The user
    ...
    if custom:
        try:
            ...
            from sff.cloud.save_identity import local_account_backup_dir
            return local_account_backup_dir(p)
        except OSError:
            pass
        except Exception:
            return Path(custom)
    ...
    try:
        from sff.cloud.save_identity import local_account_backup_dir
        return local_account_backup_dir(backup_dir)
    except Exception:
        return backup_dir
...
class CloudSaves:
    ...
    Snapshots land under %APPDATA%/SteamShip/save_backups/{appid}/,
    ...
    def list_steam_games(steam_path, steam32_id):
        ...
        Name resolution — three layers, in order:
          ...
          2. SteamShip fix_game_cache CachedAppInfo (previously fixed games)
        ...
        try:
            ...
            pass
        # --- Layer 2: SteamShip fix_game_cache (previously fixed games) ---
...
_INTERNAL_FOLDERS = frozenset({"_restore_safety", "_steamship_meta"})
...
def backup_save_location_local(entry, dest_root, log_func=None):
    """
    Backup save paths to dest_root/SteamShipAllSaves/{label}/.
    ...
    if not dest_root_norm:
        ...
        return None
    dest = dest_root_norm / "SteamShip" / _cloud_bucket() / label
    ...
    try:
        ...
        meta_path = dest / "steamship_meta.json"
...
def backup_save_location_rclone(entry, rclone_exe, remote_dest, log_func=None):
    """Upload save entry via rclone to remote_dest:SteamShipAllSaves/{label}/."""
    import subprocess
    import tempfile
    log = log_func or (lambda m: None)
    label = entry["label"]
    remote_path = _rclone_keyed_path(remote_dest, label)
    ...
    meta_tmp = Path(tempfile.mkdtemp(prefix="steamship_meta_"))
    try:
        meta_file = meta_tmp / "steamship_meta.json"
        meta_file.write_text(
            json.dumps(_entry_meta(entry, meta_sources), indent=2),
            encoding="utf-8",
        )
        subprocess.run(
            [rclone_exe, "copyto", str(meta_file), remote_path + "/steamship_meta.json",
             "--no-update-modtime"],
...
def backup_save_location_gdrive(entry, service, backup_root_id, log_func=None, folder_cache=None):
    """Upload save entry via Google Drive API. Uses flat structure: SteamShip Backups/{label}/."""
    import tempfile
    ...
    try:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="steamship_meta_"))
        try:
            meta_file = tmp / "steamship_meta.json"
...
def _parse_game_dir(game_dir, remote_root, prefix_in_remote=""):
    ...
    meta_file = game_dir / "steamship_meta.json"
...
def scan_backup_root_rclone(rclone_exe, remote_dest):
    """Scan an rclone remote for SteamShip/<id> plus legacy SteamShipAllSaves."""
    import subprocess
    import tempfile
    from sff.cloud.save_identity import cloud_relative_root, rclone_legacy_roots

    remotes = [remote_dest.rstrip("/") + "/" + cloud_relative_root()]
    remotes.extend(rclone_legacy_roots(remote_dest))
    result = {}
    all_games = []
    primary_root = remotes[0]
    tmp = Path(tempfile.mkdtemp(prefix="steamship_scan_"))
    try:
        for remote_root in remotes:
            dest = tmp / str(abs(hash(remote_root)))
            dest.mkdir(parents=True, exist_ok=True)
            subprocess.run(
                [
                    rclone_exe, "copy", remote_root, str(dest),
                    "--include", "steamship_meta.json",
                    "--fast-list",
                    "--transfers", "10",
                ],
...
def scan_backup_root_local(backup_root):
    """Scan SteamShip/<id> plus legacy SteamShipAllSaves folders."""
    base = Path(backup_root)
    roots = [
        base / "SteamShip" / _cloud_bucket(),
        base / "SteamShipAllSaves",
        base / "SteamShipAllSaves" / "Games",
    ]
    result = {}
    all_games = []
    primary = roots[0]
    for root in roots:
        ...
        if legacy_dir.is_dir() and legacy_dir not in roots:
            for item in sorted(legacy_dir.iterdir()):
                ...
                if parsed:
                    all_games.append(parsed)
    if all_games:
        result["All Backups"] = {"folder_path": str(primary), "games": all_games}
    return result
    ...
    if folder_path:
        try:
            ...
            for candidate in (p, *p.parents):
                if candidate.name == "SteamShipAllSaves":
                    return candidate / "_restore_safety"
        except Exception:
            pass
    return _BACKUP_ROOT / "SteamShipAllSaves" / "_restore_safety"
...
def _copy_restore_payload(src, dest):
    ...
    restored = 0
    for f in src.rglob("*"):
        if f.is_file() and f.name != "steamship_meta.json":
            rel = f.relative_to(src)
            target = dest / rel
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(f, target)
...
def _download_restore_source(game_entry, log):
    ...
    if rclone_path and rclone_exe:
        import subprocess
        import tempfile
        tmp = Path(tempfile.mkdtemp(prefix="steamship_restore_"))
        log("Downloading from rclone remote...")
        proc = subprocess.run(
            [rclone_exe, "copy", rclone_path, str(tmp),
             "--exclude", "steamship_meta.json", "--transfers", "10", "--fast-list"],
            capture_output=True, text=True, stdin=subprocess.DEVNULL,
            timeout=300, **_CREATE_NO_WINDOW,
        )
        if proc.returncode != 0:
