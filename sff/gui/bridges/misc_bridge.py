        ...
        try:
            ...
            if not saved_lua_path.exists():
                ...
                return _with_crack({
                    ...
                    "installed_buildid": installed_buildid,
                    "cm_buildid": cm_buildid,
                    "error": f"No saved .lua for App ID {app_id}. Steam CM did not expose public manifest IDs either, so SteamShip cannot patch this one automatically.",
                })
...
def _bridge_get_app_version(bridge):
    """Returns the current SteamShip version string."""
    ...
    def _do():
        try:
            ...
            while True:
                ...
                req = _req.Request(url, headers={"User-Agent": "SteamShip/6.1.0"})
...
def _bridge__scan_installed_games(bridge):
    ...
    from sff.core.storage.vdf import get_steam_libs
    import os
    from sff.gui.web_bridge import _collect_steamship_managed_sources
    managed_sources = _collect_steamship_managed_sources(bridge._steam_path)
            ...
            for acf in steamapps.glob("appmanifest_*.acf"):
                try:
                    ...
                    games.append({
                        ...
                        "steamship_managed": bool(managed),
                        "steamship_source": ",".join(sorted(managed)),
                    })
                except Exception as e:
...
def _bridge_steam_login(bridge):
    from sff.auth.steam_openid import login_steam
    try:
        return json.dumps(login_steam(bridge.parent()))
    except Exception as exc:
        logger.warning("steam_login failed: %s", exc)
        return json.dumps({"ok": False, "error": str(exc), "linked": False})


def _bridge_steam_logout(bridge):
    from sff.auth.steam_openid import logout_steam
    try:
        return json.dumps(logout_steam())
    except Exception as exc:
        logger.warning("steam_logout failed: %s", exc)
        return json.dumps({"ok": False, "error": str(exc), "linked": False})


def _bridge_get_steam_account(bridge):
    from sff.auth.steam_openid import steam_account_dict
    try:
        return json.dumps(steam_account_dict())
    except Exception:
        return json.dumps({"linked": False, "steamid64": "", "persona": "", "avatar": ""})


def _bridge_get_pattern_status(bridge):
    from sff.lumacore.lumacore_setup import read_pattern_chip_status
    try:
        return json.dumps(read_pattern_chip_status(bridge._steam_path))
    except Exception as exc:
        logger.debug("get_pattern_status failed: %s", exc)
        return json.dumps({"available": False, "label": "No runtime status", "action": "auto_lc_setup"})
