...
def _collect_steamship_managed_sources(steam_path, saved_lua_root=None) -> dict[str, list[str]]:
...
class WebBridge(QObject):
    ...
    def _resolve_acf(self, app_id):
        ...
        data download). Without this fallback, a SteamShip-registered game
    ...
    def apply_game_crack(self, app_id, game_name):
        ...
        def _do():
            try:
                ...
                import tempfile
                from pathlib import Path as _Path
                tmp_dir = _Path(tempfile.mkdtemp(prefix="steamship_crack_"))
    ...
    @pyqtSlot(result=str)
    @pyqtSlot(str, result=str)
    def get_steam_account(self, _arg=""):
        return _bridge_get_steam_account(self)
    @pyqtSlot(result=str)
    @pyqtSlot(str, result=str)
    def steam_login(self, _arg=""):
        return _bridge_steam_login(self)
    @pyqtSlot(result=str)
    @pyqtSlot(str, result=str)
    def steam_logout(self, _arg=""):
        return _bridge_steam_logout(self)
    @pyqtSlot(result=str)
    @pyqtSlot(str, result=str)
    def get_pattern_status(self, _arg=""):
        return _bridge_get_pattern_status(self)
    @pyqtSlot(result=str)
...
def _fetch_steam_platforms(app_ids):
    ...
    for start in range(0, len(pending), chunk_size):
        ...
        try:
            ...
            request = _req.Request(url, headers={"User-Agent": "Mozilla/5.0 SteamShip"})
...
def _fetch_steam_image_urls(app_ids):
    ...
    try:
        ...
        request = _req.Request(url, headers={"User-Agent": "SteamShip/5.4.0"})
    ...
    try:
        ...
        for _ in range(3):
            try:
                ...
                _req2 = _req.Request(f"{_base}?{_qs}", headers={"User-Agent": "SteamShip/6.1.0"})
