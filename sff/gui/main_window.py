...
class SFFMainWindow(QMainWindow):
    def __init__(self, ui, steam_path):
        ...
        self._current_theme = _saved_theme if (_saved_theme and _saved_theme in THEMES) else "harbor"
        ...
        # <sff_data>/manifests/ already holds every manifest SteamShip has
    ...
    def _build_classic_ui(self, steam_path):
        ...
        add_help_button(layout, "Main Hub", "SteamShip Main Hub\n\n"
            ...
            "  - Context menu: Add/remove SteamShip from Windows Explorer\n"
            "    right-click menu.",
            parent_widget=self,
        )
        ...
        try:
            ...
            if failed:
                try:
                    ...
                    QMessageBox.warning(self, "SteamShip update — issue", msg)
                except Exception:
                    pass
        except Exception:
    ...
    def _install_web_splash(self):
        ...
        for candidate in ("steamship.png", "steamship.ico", "SFF.png", "SFF.ico"):
            try:
    ...
    def _on_web_view_load_finished(self, ok: bool):
        if not ok:
            self._web_view.setHtml(
                ...
                "<div><h2 style='color:#c9a227'>SteamShip UI failed to load</h2>"
    ...
    def _apply_setting_live(self, s, parent_widget=None):
        ...
        if s == Settings.PLAY_MUSIC:
            ...
            if parent_widget:
                QMessageBox.information(
                    parent_widget,
                    "Restart Recommended",
                    "Steam path changed. Please restart SteamShip for all changes to take full effect.",
                )
        ...
        elif s == Settings.STEAMSHIP_PT_REPO:
            try:
                from sff.lumacore.lumacore_setup import ensure_pattern_fetch_mirror
                if self.steam_path:
                    ensure_pattern_fetch_mirror(Path(self.steam_path), None)
            except Exception:
                logger.exception("STEAMSHIP_PT_REPO mirror write failed")
    ...
    def closeEvent(self, event):
        ...
        if (
            ...
            if not self._tray_hide_notified:
                ...
                self._tray.notify(
                    "SteamShip",
                    "SteamShip is running in the system tray. Click the ^ arrow near the clock to find it.",
                )
        else:
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            ...
            "About SteamShip",
            f"SteamShip\nVersion {VERSION}\n\n"
            "https://github.com/haker146/SteamShip/releases",
        )
