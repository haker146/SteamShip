...
"""
    ...
    def _find_gse_exe():
        ...
        3. %APPDATA%/SteamShip/gse_tool/ (previously staged persistent copy)
        ...
        p = appdata / "SteamShip" / "gse_tool" / "generate_emu_config.exe"
        if p.exists():
            return p
        return None
    ...
    def _run_gse_config(
        ...
        Copies the tool to %APPDATA%/SteamShip/gse_tool/ before running so it
        ...
        if not config_exe:
            ...
            log("  Searched: sys._MEIPASS, project root, %APPDATA%/SteamShip/gse_tool/")
            return False
        ...
        appdata = Path(os.environ.get("APPDATA") or str(Path.home() / "AppData" / "Roaming"))
        run_dir = appdata / "SteamShip" / "gse_tool"
