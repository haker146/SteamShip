...
# executables to unpack. These are SteamShip's own backup / staging
...
SKIP_DIR_NAMES = {
        ".steamship_exe_backups",  # created by sff/game/steamauto.py
...
class SteamStubUnpacker:
    ...
    def _find_steamless():
        ...
        if sys.platform == "win32":
            ...
            if appdata:
                candidates.append(Path(appdata) / "SteamShip" / "tools" / STEAMLESS_EXE)
        for p in candidates:
            if p.exists():
                return str(p)
    ...
    def _should_skip(self, exe_path):
        """Skip installers, redistributables, and SteamShip backup folders."""
    ...
    def restore_directory(self, directory, log_func=None):
        ...
        Returns the count of files restored. Skips SteamShip's own
        backup folders (`.steamship_exe_backups/`, `.steamlocked.bak/`,
