...
--
-- STEAMSHIP_EXCLUDED_DEPOTS: {depot_json}
--
-- Managed by SteamShip. Toggle in Settings to remove.
...
def get_excluded_depots(steam_path: Path) -> set[str]:
    ...
    for line in text.splitlines():
        if "STEAMSHIP_EXCLUDED_DEPOTS:" not in line:
            continue
        raw = line.split("STEAMSHIP_EXCLUDED_DEPOTS:", 1)[1].strip()
        try:
