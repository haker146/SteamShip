...
def get_steamauto_cli_path():
    ...
    #    SteamShip_GUI.exe (e.g. .\third_party\SteamAutoCrack\cli\SteamAutoCrack.CLI.exe).
    root = root_folder()
    for subpath in _EXE_PATHS:
        p = root / subpath
        if p.exists():
...
def _snapshot_executables(game_path):
    ...
    """
    backups = {}
    backup_dir = game_path / ".steamship_exe_backups"
    ...
    for original, backup in backups.items():
        if not original.exists():
            ...
            if backup.exists():
                ...
                print_func(
                    f"[SteamShip] RESTORED {original.name} — SteamAutoCrack "
                    "removed it without producing a patched version."
                )
                restored += 1
            else:
                print_func(
                    f"[SteamShip] WARNING: {original.name} was removed and "
...
def _write_run_config(cli_dir: Path, mode: str) -> Path | None:
    ...
    out_name = "config.steamless_only.json" if mode == "steamless_only" else "config.steamship.json"
...
def run_steamauto(
    ...
    *,
    ...
    try:
        ...
        if mode == "steamless_only":
            print_func("[SteamShip] SteamAutoCrack: STEAMLESS-ONLY mode (no Goldberg / no EMU).")
    except Exception as exc:
        print_func(f"[SteamShip] Could not write SteamAutoCrack config: {exc}")
        return 96

    # Safety: snapshot all game executables before the CLI touches them
    print_func("[SteamShip] Backing up game executables before cracking...")
    backups = _snapshot_executables(game_path)
    if backups:
        print_func(f"[SteamShip] Backed up {len(backups)} executable(s).")
    else:
        print_func("[SteamShip] No executables found in game directory.")
    ...
    if restored > 0:
        print_func(
            f"\n[SteamShip] WARNING: {restored} executable(s) were restored "
    ...
    if crash_detected:
        print_func(
            "\n[SteamShip] SteamAutoCrack CLI crashed because its bundled "
