...
def _steamship_pattern_raw_bases() -> list[str]:
    try:
        from sff.brand import PATTERN_MIRROR_BRANCH, pattern_mirror_repo
    ...
    if not repo or "/" not in repo:
        return []
    bases = [f"https://raw.githubusercontent.com/{repo}/{branch}"]
    if branch != "main":
        bases.append(f"https://raw.githubusercontent.com/{repo}/main")
    bases.append(f"https://cdn.jsdelivr.net/gh/{repo}@{branch}")
    if branch != "main":
        bases.append(f"https://cdn.jsdelivr.net/gh/{repo}@main")
    return bases


def _download_pattern_body(subdir: str, sha: str) -> Optional[str]:
    rel_sub = f"{subdir}/{sha}.toml" if subdir else f"{sha}.toml"
    urls: list[str] = []
    for base in _steamship_pattern_raw_bases():
        urls.append(f"{base}/{rel_sub}")
    urls.extend((
        f"{_PATTERN_REPO_RAW}/{rel_sub}",
        f"{_PATTERN_REPO_CDN}/{rel_sub}",
    ))
    ...
    try:
        resp = httpx.get(
            ...
            params={"branch": "pattern", "file": rel_sub},
...
def _bundled_pattern_body(subdir: str, sha: str) -> Optional[str]:
    try:
        from sff.brand import bundled_pattern_dir
        root = bundled_pattern_dir()
    except Exception:
        return None
    if not root.is_dir():
        return None
    candidates = []
    if subdir:
        candidates.append(root / subdir / f"{sha}.toml")
    candidates.append(root / f"{sha}.toml")
...
def _upsert_pattern_mirror(text: str, url: str) -> str:
    ...
    for line in lines:
        ...
        if stripped.startswith("[") and stripped.endswith("]"):
            if in_section and not found_mirror:
                out.append(f'mirror = "{url}"')
                found_mirror = True
            in_section = stripped.lower() == "[pattern_fetch]"
            if in_section:
                found_section = True
            out.append(line)
            continue
        if in_section and stripped.lower().startswith("mirror"):
            out.append(f'mirror = "{url}"')
            found_mirror = True
            continue
        out.append(line)
    ...
    if not found_section:
        if out and out[-1].strip():
            out.append("")
        out.append("[pattern_fetch]")
        out.append(f'mirror = "{url}"')
    return "\n".join(out).rstrip() + "\n"


def ensure_pattern_fetch_mirror(steam_path: Path, callback: Optional[Callable[[str], None]] = None) -> None:
    """Point LumaCore.dll at the SteamShip TOML repo; KoriaPolis stays fallback."""
    try:
        from sff.brand import pattern_mirror_template
        url = pattern_mirror_template()
    except Exception:
        url = ""
    if not url:
        return
    cfg = steam_path / "lumacore.toml"
    try:
        existing = cfg.read_text(encoding="utf-8") if cfg.is_file() else ""
        updated = _upsert_pattern_mirror(existing, url)
        if updated != existing:
            cfg.write_text(updated, encoding="utf-8", newline="\n")
            _progress(f"Wrote pattern_fetch.mirror → {url}", callback)
    except OSError as exc:
        _progress(f"Could not write lumacore.toml mirror: {exc}", callback)
...
def _prewarm_lumacore_patterns(
    ...
    ensure_pattern_fetch_mirror(steam_path, callback)
    jobs = (
    ...
    for label, binary_path, subdir in jobs:
        ...
        sha_name = f"{sha}.toml"
        if subdir == "steamclientipc":
            cache_targets = [cache_root / "steamclientipc" / sha_name]
        else:
            cache_targets = [cache_root / sha_name]
        if all(t.is_file() and t.stat().st_size > 0 for t in cache_targets):
            _progress(f"Pattern cache ready for {label}.", callback)
            continue

        body = _bundled_pattern_body(subdir, sha)
        if body is None:
...
def read_pattern_chip_status(steam_path) -> dict:
    """Small Home-chip payload from <Steam>/lumacore/status.json."""
    result = {
        "available": False,
        "hooks_ok": False,
        "toml_ok": False,
        ...
        "detail": "",
        "action": "auto_lc_setup",
    }
    try:
        path = Path(steam_path) / "lumacore" / "status.json"
        payload = json.loads(path.read_bytes())
    except Exception:
        dll = Path(steam_path) / "LumaCore.dll"
        if not dll.is_file():
            result["label"] = "Runtime missing"
            result["detail"] = "Install SteamShip runtime so Steam can see added games."
        return result
    toml = payload.get("toml_found") or {}
    toml_ok = bool(toml.get("steamclient")) and bool(toml.get("steamui"))
    hooks = int(payload.get("hooks_installed") or 0)
    missed = payload.get("hooks_missed") or []
    ipc_path = Path(steam_path) / "lumacore" / "pattern" / "steamclientipc"
    sha = str(payload.get("steamclient_sha") or "")
    ipc_ok = True
    if sha:
        ipc_ok = (ipc_path / f"{sha}.toml").is_file()
    result.update({
        "available": True,
        "hooks_ok": hooks > 0 and not missed,
        "toml_ok": toml_ok,
        "ipc_ok": ipc_ok,
    ...
    if not toml_ok:
        result["label"] = "Missing TOML"
        result["detail"] = "Pattern files for this Steam build are missing."
        result["action"] = "auto_lc_setup"
    elif not ipc_ok:
        result["label"] = "No IPC pattern"
        result["detail"] = "steamclientipc TOML is missing for this build."
        result["action"] = "auto_lc_setup"
    elif hooks <= 0:
        result["label"] = "Hooks not installed"
        result["detail"] = "Start Steam once with internet, then reopen SteamShip."
        result["action"] = ""
    else:
        result["label"] = "Hooks OK"
        result["detail"] = f"{hooks} hooks · Steam {result['build_id']}"
        result["action"] = ""
    return result
...
def get_installed_lumacore_version(steam_path: Path) -> str:
    ...
    SteamShip last installed, falling back to the version LumaCore itself
...
def remember_installed_lumacore_version(version: str) -> None:
    """Persist the version label SteamShip just installed."""
...
# ─────────────────────────────────────────────────────────────────────────
#
# Removes every file SteamShip installs as part of LumaCore. Steam MUST be
