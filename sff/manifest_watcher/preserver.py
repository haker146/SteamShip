...
uninstalls a game from the Steam UI. SteamShip has its own canonical
copy of every manifest it ever downloaded, sitting in the writable
staging directory next to SteamShip.exe (`<sff_data>/manifests/`),
...
def restore_manifest(deleted_path: Path) -> bool:
    ...
    try:
        ...
        # when SteamShip runs as admin and Steam doesn't. Every restore
        ...
        # can't fix it from inside SteamShip, so log at debug and move on.
