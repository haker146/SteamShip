...
class UI:
    ...
    def check_updates(self, os_type, test = False):
        ...
        def _do_auto_update():
            ...
            # If zip had a single top-level folder (e.g. SteamShip-v4.5.3/), flatten so copy source is the contents
        ...
        def _do_windows_frozen_update():
            ...
            updater_bat.write_text(
                ...
                "if %errorlevel% GEQ 8 (\n"
                "  echo Robocopy error! Update may be incomplete. Check your SteamShip folder.\n"
                "  pause\n"
                "  goto :end\n"
                ")\n"
            ...
            print(Fore.GREEN + "Update started. SteamShip will restart automatically." + Style.RESET_ALL, flush=True)
            os._exit(0)
        ...
        def _do_linux_frozen_update():
            ...
            install_sh = tmp_update / "steamship_install.sh"
            if not install_sh.exists():
                print(Fore.RED + "steamship_install.sh not found in update package." + Style.RESET_ALL)
                logger.warning("Linux update: steamship_install.sh missing from %s", tmp_update)
                return
            ...
            install_cmd = f"cd {shlex.quote(str(tmp_update))} && bash steamship_install.sh; echo; echo 'Press Enter to close...'; read"
            terminals = [
            ...
            if launched:
                print(Fore.GREEN + "Install script launched. SteamShip will close now." + Style.RESET_ALL)
                time.sleep(0.5)
                sys.exit(0)
            else:
                print(Fore.RED + "Could not launch installer. Run manually:" + Style.RESET_ALL)
                print(f"  cd {tmp_update} && bash steamship_install.sh")
