...
echo ========================================
echo Building SteamShip Executable
echo ========================================
echo.
...
echo ========================================
echo.
echo Folder:     dist\SteamShip\
echo Executable: dist\SteamShip\SteamShip.exe
echo.

if exist "dist\SteamShip\SteamShip.exe" (
    python -c "import os; size = sum(os.path.getsize(os.path.join(r,f)) for r,d,files in os.walk('dist/SteamShip') for f in files); print(f'Total size: {size / (1024*1024):.1f} MB')"
)

echo.
echo You can now run: dist\SteamShip\SteamShip.exe
echo Zip the dist\SteamShip\ folder for distribution.
