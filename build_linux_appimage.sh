#!/usr/bin/env bash
# SteamShip AppImage build script
...
echo "    Build output: dist/SteamShip_GUI/"
...
cp -r dist/SteamShip_GUI/* "${APPDIR}/usr/bin/"

if [ -f "steamship.png" ]; then
    cp steamship.png "${APPDIR}/${APP_NAME}.png"
elif [ -f "SFF.png" ]; then
    cp SFF.png "${APPDIR}/${APP_NAME}.png"
else
    echo "    WARNING: steamship.png not found — AppImage will launch without an icon."
fi
...
exec "$HERE/usr/bin/SteamShip_GUI" "$@"
APPRUN_EOF
