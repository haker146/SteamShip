...
This document describes every subsystem in LumaCore, its purpose, the Steam internals it touches, and the configuration interface exposed to SteamShip via Lua scripts.
...
1. **Local cache** — `<Steam>\lumacore\pattern\<sha>.toml` (steamclient / steamui) or `<Steam>\lumacore\pattern\steamclientipc\<sha>.toml` (IPC). A parseable cache hit skips the network.
2. **User mirror** (optional). If `[pattern_fetch] mirror` is set in `lumacore.toml`, the fetcher substitutes `{subdir}` (`steamclient`, `steamui`, or `steamclientipc`) and `{sha}` into the URL. SteamShip writes this to `haker146/SteamShip` branch `pattern`. Any failure (HTTP 4xx/5xx, network error, parse error) logs a debug line and falls through.
3. **GitHub raw** — `raw.githubusercontent.com/KoriaPolis/Steam-Auto-PT/pattern/<subdir>/<sha>.toml`.
4. **jsDelivr CDN** — `cdn.jsdelivr.net/gh/KoriaPolis/Steam-Auto-PT@pattern/<subdir>/<sha>.toml`. Used only on transport failure since GitHub raw and jsDelivr serve the same content; a 404 from either short-circuits the remaining network legs.

A successful network fetch is written through to the local cache.
...
When Steam mounts a depot it calls this function to fetch the AES-128 decryption key for that depot from the user's license data.  The hook intercepts the call, checks whether `LuaLoader` has a key for the requested depot ID (loaded from the `.lua` script provided by SteamShip), and writes it into the output buffer.  If no key is known, the call falls through to the original function.
...
SteamShip writes `.lua` files to `Steam\config\stplug-in\<appid>.lua`.  LumaCore watches this directory and reloads files as they change.
...
Placed in the Steam installation directory.  SteamShip writes this file during LumaCore setup.
