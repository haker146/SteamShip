    ...
    namespace {
        ...
        bool FetchFromNetwork(const std::string& sha, std::string& bodyOut) {
            if (!Settings::patternMirror.empty()) {
                const std::string mirrorUrl = ApplyMirrorTemplate(
                    Settings::patternMirror, kIPCSubdir, sha);
                LOG_IPC_DEBUG("IpcLoader: fetching ipc_methods from user-mirror {}", mirrorUrl);
                auto resp = RuntimeHttp::Get(mirrorUrl);
                if (!resp.networkError && resp.status == 200 && !resp.body.empty()) {
                    bodyOut = std::move(resp.body);
                    return true;
                }
                LOG_IPC_DEBUG("IpcLoader: user-mirror missed for ipc (status={}); "
                              "falling through to KoriaPolis", resp.status);
            }
